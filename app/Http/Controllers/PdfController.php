    ...
    public function index()
    {
        ...
        foreach ($user->adresses as $adresse) {
            if ($adresse->napi) {
                $presencenapi = true;
                $adressenapi = $adresse;
                $this->fpdf->Cell(75, 10, $adresse->adresse, 0, 1, 'L', 1);
                $this->fpdf->Cell(75, 10, $adresse->code_postal . ' ' . $adresse->ville, 0, 1, 'L', 1);

                $this->fpdf->Ln(8); // saut de ligne 10mm
                $this->fpdf->Cell(80, 10, 'N° API : ' . $adresse->napi, 0, 1, 'L', 1);
                $this->fpdf->Cell(80, 10, 'N° SIRET : ' . $adresse->siret, 0, 1, 'L', 1);
            }
            ...
            foreach ($user->ruchers as $rucher) {
                $this->fpdf->Cell(100, 10, $rucher->nom_rucher, 0, 1, 'L', 1);
                //bordure de séparation 	
                $this->fpdf->Cell(100, 0, '', 1, 1, 'L', 1);
                
                foreach ($rucher->ruches as $ruche) {
                    $this->fpdf->Cell(100, 10, $ruche->nom_ruche, 0, 1, 'L', 1);
                }
            }
        }
